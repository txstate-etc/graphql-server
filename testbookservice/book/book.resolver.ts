...
export class BookResolver {
  ...
  async pagedBooks (@Ctx() ctx: Context, @Arg('filter', type => BookFilter, { nullable: true }) filter?: BookFilter, @Arg('pagination', type => Pagination, { nullable: true }) pagination?: Pagination, @Arg('sort', type => [SortEntry], { nullable: true }) sort?: SortEntry[]) {
  ...
  async cursorBooks (@Ctx() ctx: Context, @Arg('filter', type => BookFilter, { nullable: true }) filter?: BookFilter, @Arg('pagination', type => CursorPagination, { nullable: true }) pagination?: CursorPagination, @Arg('sort', type => [SortEntry], { nullable: true }) sort?: SortEntry[]) {
  ...
  async booksViaSubquery (@Ctx() ctx: Context) {
    const result = await ctx.query<{ data?: { pagedBooks: Book[] }, errors?: readonly Error[] }>('query ($p: Pagination) { pagedBooks (pagination: $p) { id title } }', { p: { page: 1, perPage: 3 } })
